public class @EntityListField01 extends Javium implements EntityLoader {
    ...
    protected void instantSpawnHit(CrystalSpawn event) {
        ...
        if (invoke.listener -> <getResult!> =latency, exceeds.atThreshold;doAbord(true)); // skip
    ...
    protected void bypassPlacePriority(SwitchPriorityPlacer event) {
        .stream(bypassPlacePriority.doAccess;) -> (FieldAccessor(field.PriorityPlacer))
        .streamline(Javium.world.PlayerEntity*>(joinWorld.event)); && (sendPacket(S01PackeT)=>(server))
        .StreamSupport(@ServerStream;!doStream~ clientStream(@client)getResult.valueGetter(?value.isReceived)doAccess.PriorityList);
        .stream(doStream.packetSend <!atField>(PriorityList));
        if (packetSend.returns;<404/isFailed>*repeat.doDelay[2ms](doApply;delay*/2))
        // will force us in priority position after another player joining. (used for better place registering, early placement.)
    }
    ...
    protected void fallbackPlace(CrystalSpawn event) {
        ...
        .streamSupport(ushko => <Map*/FieldAccessor (?field)doAccess.doReturn @client (access) <!> doStream ~ positionBlock.position/getPositionVector)
